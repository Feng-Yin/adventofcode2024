...
def get_shortest_path(maze, start, end):
    ...
    return pathes[pos_to_str(start, end)], costes[pos_to_str(start, end)] - 1
...
def q1(maze, start, end, walls):
    path, base = get_shortest_path(maze, start, end)
    print("base:", base)
    #new_pathes = run_parallel(maze, start, end, walls)
    new_pathes = []
    for i in range(len(walls)):
        print("Progress: ", i+1, "/", len(walls), sep='')
        p1, p2 = get_valid_sides(maze, walls[i])
        p1_i = path.index(p1)
        p2_i = path.index(p2)
        new_pathes.append(base - abs(p1_i - p2_i) + 2)
    #    maze_copy = copy.deepcopy(maze)
    #    #maze_copy = maze.copy()
    #    maze_copy[walls[i][0]][walls[i][1]] = '.'
    #    #print("Removed ", wall)
